...
Prisma with SQLite supports `updateMany` and `deleteMany`. So if your `where`
clause matches more than one record, your update will apply to every record that
matches. However, `upsertMany` is not supported.
